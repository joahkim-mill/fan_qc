...
import torch 
# # 
# # COMPUTE WITH THE SPRING DATA
# #region compute average of good fans
# X = torch.load('spring_good.pt')  
# good = X.numpy()
# bad = torch.load('spring_bad.pt').numpy()
# freq = torch.load('frequency.pt')
# freq = freq.numpy()


# excl = np.vstack((good[:61,:], good[62:,:]))
# avg = np.mean(excl, axis=0)
# std = np.std(excl, axis=0)

# # avg = good[13,:]  # use fan 14 as standard
# # std = np.zeros(avg.shape)
# # avg = np.mean(good, axis=0)
# # std = np.std(good, axis=0)

# # TRY: taking average excluding the bad fans
# # good_ok = 1, 4-5, 7-9, 11-18, 20-22, 24-27, 31-32, 36-38, 40-44, 46-51, 53-54, 56-57, 59-61, 64, 66
# # good_ok = np.vstack(( good[0,:], good[3:5,:], good[6:9,:], good[10:18,:], good[19:22,:], good[23:27,:], good[30:32,:], good[35:38,:], good[39:44,:], good[45:51,:], good[52:54,:], good[55:57,:], good[58:61,:], good[63,:], good[65,:]))
# # avg = np.mean(good_ok, axis=0)
# # std = np.std(good_ok, axis=0)
